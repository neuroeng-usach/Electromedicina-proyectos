::: minipage
**Análisis del rendimiento diagnóstico y limitaciones técnicas de la
Tomografía por Electroimpedancia para la detección del cáncer de mama:
Revisión Bibliográfica**\

Renata Bucarel Salvatierra $^{1}$, Antonia Contreras Naredo$^{2}$,
Renata Contreras Mancilla$^{3}$\
*Universidad de Santiago de Chile. Facultad de Ingeniería, Ingeniería
Civil Biomédica.*\
:::
...
**Palabras Claves---Tomografía por impedancia eléctrica, cáncer de mama,
...
siendo la mayor tasa entre las mujeres adultas [@OMS2026]. Durante el
...
periodo [@IARC]. En Chile, la realidad no es muy distinta y llega a ser
...
equitativo a tratamientos efectivos [@PairoCuba2025]. La detección
...
[@Ruiz2026a; @NCI2025]. Sin embargo, su desempeño puede disminuir para
...
tejidos dificulta la visualización de lesiones [@Mann2022]. Por otra
...
métodos complementarios o alternativos [@ACS2026].
...
mamografía la califican como leve a dolorosa [@Keemers2000], mientras
que otro estudio reveló que la incomodidad o dolor durante el examen
afecta la satisfacción del paciente [@Engelman2005]. Estas molestias son
...
[@Mendat2017; @Uscher2025]. Estos factores, influyen al momento de
...
por lo que significa [@Ruiz2026a; @Dois2021].
...
paralelo a otras técnicas [@Pennati2023].
...
baja intensidad [@Hope2004]. Las principales propiedades involucradas
...
tejido [@Pennati2023]. Debido a que estas propiedades varían entre
distintos tipos de tejido, es posible utilizarlas como biomarcadores
para generar imágenes funcionales mediante EIT [@Hope2004].
...
eléctrico [@Pennati2023]. Por su parte, la impedancia corresponde a la
oposición total que presenta un tejido al paso de corriente alterna,
integrando tanto componentes resistivos como capacitivos [@Hope2004].
...
de imágenes por EIT [@Pennati2023].
...
tumores malignos [@Zhao2014]. Diversas investigaciones han demostrado
...
lesiones benignas [@Zhao2014]. Este contraste eléctrico es el principio
biofísico que permite identificar regiones sospechosas durante la
reconstrucción de imágenes mediante EIT [@Scholz2000].

El aumento de conductividad observado en los tumores malignos se explica
por cambios característicos del microambiente tumoral [@Zhao2014]. Las
...
[@Scholz2000]. Como consecuencia, los tumores malignos exhiben una
conductividad significativamente superior a la del tejido adiposo y del
tejido mamario normal [@Zhao2014].
...
potencial eléctrico medida en la superficie [@Pennati2023]. Estas
...
espacial de conductividad dentro de la mama [@Ganesan2023]. De esta
...
partir de mediciones realizadas sobre su superficie [@Pennati2023]. Su
...
potencial generadas en los electrodos restantes [@Ganesan2023]. Estas
...
utilizada para reconstruir una imagen funcional [@Pennati2023].
...
[@Ganesan2023]. Durante cada ciclo de adquisición, un par de electrodos
...
esa corriente a través del tejido [@Pennati2023]. Este procedimiento se
...
mama desde múltiples posiciones [@Ganesan2023].

La reconstrucción de la imagen se basa en la resolución del denominado
problema inverso de la EIT [@Pennati2023]. En el problema directo se
...
distribución de conductividad que los produjo [@Ganesan2023]. Debido a
...
estables y físicamente consistentes [@Pennati2023].

::: center
![image](./fig_1.png){width="\\columnwidth"} []{#fig:matches
label="fig:matches"}
:::
...
mamario [@Ganesan2023]. Las regiones con mayor conductividad, asociadas
...
conductividad inferiores [@Zhao2014]. Por esta razón, la EIT entrega
...
tecnológico [@Pennati2023]. Es una técnica que no utiliza radiación
...
tiempo real [@Pennati2023].

A pesar de estas ventajas, la principal limitación de la EIT continúa
siendo su baja resolución espacial [@Pennati2023]. La corriente
...
con precisión estructuras pequeñas o profundas [@Pennati2023].
...
[@Pennati2023]. Aunque existen diferentes implementaciones, la
...
electrodos [@Ganesan2023]. Mantener la corriente constante es
...
variaciones en la excitación aplicada [@Pennati2023].
...
[@Ganesan2023]. El multiplexor selecciona el par de electrodos que
...
utilizados para medir los voltajes superficiales [@Ganesan2023]. Este
...
[@Pennati2023]. Para ello se emplean amplificadores de instrumentación,
que incrementan la amplitud de la señal y reducen la influencia del
ruido y de las interferencias de modo común [@Ganesan2023].
...
microcontrolador o un computador [@Ganesan2023].
...
[@Pennati2023]. En sistemas de EIT, esta etapa suele implementarse
...
Finalmente, la imagen reconstruida puede someterse a un post
procesamiento para mejorar su interpretación clínica [@Ganesan2023].
...
lesiones de pequeño tamaño [@Ganesan2023]. Estas etapas no modifican las
...
[@Pennati2023]. Entre los factores con mayor influencia se encuentran el
...
[@Ganesan2023]. La variación de estos parámetros puede modificar la
resolución de la imagen reconstruida y, en consecuencia, afectar el
desempeño diagnóstico de la técnica [@Mansouri2022].
...
[@Xu2021]. En los estudios sobre EIT mamaria, las métricas más
...
[@Xu2021]. Una sensibilidad elevada reduce la probabilidad de falsos
...
[@Rezanejad2022]. Por su parte, la especificidad representa la
proporción de pacientes sin la enfermedad que obtienen un resultado
negativo [@Xu2021]. Una alta especificidad disminuye la cantidad de
falsos positivos y mejora la capacidad de la prueba para diferenciar
lesiones benignas de lesiones malignas [@Rezanejad2022].
...
reconstrucción de imágenes, se emplearon sólamente como apoyo para
...
frecuencia), método de adquisición, algoritmo de reconstrucción, post
procesamiento de imágenes y validación, permitiendo identificar
...
[@Pennati2023; @Hope2004; @Rixen2023]. Desde una perspectiva teórica y
biofísica, Scholz y Anderson [@Scholz2000] establecieron las bases de la
...
Zhao et al. [@Zhao2014] diseñaron y simularon un sistema de mamografía
...
[@Mansouri2022] mediante modelos tridimensionales por elementos finitos
...
inverso [@Mansouri2022].

A nivel de instrumentación física y software de reconstrucción, Ganesan
y Durgamahanthi [@Ganesan2023] implementaron un banco de pruebas
...
al. [@Pennati2023] examinaron las arquitecturas de adquisición
...
altas y esquemas diferenciales de detección [@Pennati2023]. Finalmente,
abordando las limitaciones de tiempo de cómputo asociadas a la inversión
iterativa matricial, Rixen et al. [@Rixen2023] exploraron la integración
...
Hessiana y acelerando la clasificación diagnóstica [@Rixen2023].

De manera complementaria, Gómez-Cortés et al. [@GomezCortes2022]
...
::::: table*
::: tabularx
@ Y0.75 Y1.05 Y0.85 Y0.80 Y1.15 Y1.40 @

**Estudio** & **Tipo de modelo/ sistema** & **N.º de electrodos** &
**Parámetros de excitación** & **Algoritmo/ método** & **Hallazgo
principal**\

Scholz y Anderson (2000) [@Scholz2000] & Simulación numérica FEM 2D/3D &
Malla de frontera continua & Baja corriente AC & Ecuación de
Laplace/campos de corriente & Valida la detectabilidad del contraste
dieléctrico en frontera.\
Zhao et al. (2014) [@Zhao2014] & Trans-admitancia mamaria (TAM)/FEM &
Matriz planar $60 \times 60$ (3.600 canales) & 10 Hz--1 MHz & Inversión
de matriz de admitancia & Mejora la resolución lateral y la estimación
de profundidad.\
Mansouri et al. (2022) [@Mansouri2022] & Simulación 3D con *fantoma*
cónico (FEM) & 16, 24, 32 y 40 (estático, dinámico) & 0,9 mA & Métodos
de campo elíptico & 40 electrodos en modo dinámico logran el menor
RMSE.\
Ganesan y Durgamahanthi (2023) [@Ganesan2023] & Prototipo DAQ +
*fantoma* salino & 16 electrodos de cobre (anillo) & 1 mA (VCCS) &
Gauss--Newton vs. Variación Total + Canny/K-means & TV entrega bordes
nítidos; el postprocesamiento define anomalías.\
Pennati et al. (2023) [@Pennati2023] & Revisión instrumental DAQ y
*wearables* & Arreglos variables adaptables & Multifrecuencia/AC de baja
amplitud & Reconstrucción diferencial temporal & Alta resolución
temporal; necesidad de mitigar la impedancia de contacto.\
Rixen et al. (2023) [@Rixen2023] & Simulación/*Machine Learning* &
Arreglo multicanal estándar & Patrones de inyección combinados &
Clasificadores de aprendizaje automático & Clasificación directa sin
requerir inversión matricial iterativa.\
:::

::: minipage
*Nota:* FEM: Método de Elementos Finitos; VCCS: Fuente de Corriente
Controlada por Voltaje; DAQ: Sistema de Adquisición de Datos; GN:
Gauss--Newton; TV: Variación Total; RMSE: Error Cuadrático Medio; TAM:
Mamografía por Trans-admitancia.
:::
:::::
...
tradicionales [@Rezanejad2022; @Ganesan2023]. En el análisis
cuantitativo de mayor escala disponible, Rezanejad Gatabi et al.
[@Rezanejad2022] llevaron a cabo un metaanálisis sistemático que compiló
...
[@Rezanejad2022] analizaron el desempeño de un sistema de mamografía por
...
iniciales [@Rezanejad2022].

Por otra parte, en una investigación realizada por Xu et al. [@Xu2021]
...
por Hope e Iles [@Hope2004], quienes evaluaron el impacto del escaneo
...
[@Rezanejad2022; @Hope2004]. A continuación se presenta una tabla
resumen con las distintas características y parámetros obtenidos en cada
estudio revisado.

::::: table*
::: tabularx
@ Y1.05 Y0.90 Y0.65 Y1.25 Y0.70 Y0.80 Y1.55 @

**Estudio** & **Tipo de estudio** & **Muestra** & **Estándar de
referencia** & **Sensibilidad** & **Especificidad** & **Conclusión
clínica**\

Rezanejad Gatabi et al. (2022) [@Rezanejad2022] & Metaanálisis (12
estudios) & 5.487 pacientes & Histopatología / Mamografía / Ecografía &
75,88% & 82,04% & Capacidad diagnóstica global sólida con alta
heterogeneidad técnica.\
Xu et al. (2021) [@Xu2021] & Estudio retrospectivo & 645 pacientes &
Histopatología y seguimiento clínico & 80,1% & 75,1% & Exactitud de
77,2%. Comparación directa con mamografía y ecografía.\
Hope e Iles (2004) [@Hope2004] & Revisión clínica diagnóstica &
Múltiples cohortes & Biopsia / Mamografía & Variable según tipo de
lesión & Alta en lesiones BI-RADS 3/4 & Utilidad complementaria para
disminuir biopsias innecesarias.\
:::

::: minipage
*Nota:* IC 95%: Intervalo de Confianza al 95%; BI-RADS: *Breast Imaging
Reporting and Data System*; EIS: Escaneo por Impedancia Eléctrica.
:::
:::::
...
por Electroimpedancia mamaria [@Pennati2023]. Los estudios analizados
...
comparaciones directas entre los sistemas evaluados [@Ganesan2023]. Esta
variabilidad metodológica y heterogeneidad, también identificada por
Gómez-Cortés [@GomezCortes2022], constituye una de las principales
limitaciones para interpretar el desempeño diagnóstico reportado en la
literatura [@Rezanejad2022].
...
[@Pennati2023]. En general, un mayor número de electrodos aumenta la
...
reconstrucción [@Mansouri2022]. Sin embargo, no todos los estudios
...
diseño específico de cada sistema [@Ganesan2023].

Por ejemplo, al contrastar las arquitecturas de adquisición, se
evidencia que mientras Ganesan y Durgamahanthi [@Ganesan2023] utilizaron
un arreglo de 16 electrodos obteniendo contornos difusos que requirieron
filtros morfológicos posteriores, Mansouri et al. [@Mansouri2022]
...
empleados para estimar la distribución de conductividad [@Ganesan2023].
...
discontinuidades entre tejidos [@Pennati2023]. Como consecuencia, dos
...
de la calidad de las imágenes obtenidas [@Ganesan2023].
...
solución salina y prototipos instrumentales [@Mansouri2022]. Estas
...
algoritmo de reconstrucción [@Ganesan2023]. Esta falta de
...
EIT [@Pennati2023]. Uno de los factores con mayor impacto es el número y
...
[@Mansouri2022]. Configuraciones con una mayor densidad de electrodos
proporcionan una cobertura espacial más completa del tejido y disminuyen
la incertidumbre de la reconstrucción [@Mansouri2022]. Sin embargo, este
...
[@Ganesan2023]. Los algoritmos disponibles utilizan estrategias
...
dentro de la cadena de reconstrucción [@Ganesan2023]. Técnicas como la
...
la imagen reconstruida [@Ganesan2023]. Aunque estas herramientas mejoran
...
uniforme adoptado por todos los estudios [@Pennati2023].
...
de las imágenes [@Ganesan2023]. Esta dependencia de múltiples
...
desempeño de la técnica en un contexto clínico [@Rezanejad2022].
...
las métricas reportadas [@Rezanejad2022]. En términos generales, la
...
[@Rezanejad2022]. Las investigaciones emplean distintos estándares de
...
con características clínicas y rangos etarios diferentes [@Hope2004].
...
[@Rezanejad2022]. Esto demuestra que las métricas diagnósticas no pueden
extrapolarse directamente entre estudios desarrollados bajo metodologías
diferentes [@Hope2004]. Por otro lado, si bien el estudio de Xu et al.
[@Xu2021] incluye una comparación directa, ya que evaluó la EIT, la
...
del proceso diagnóstico del cáncer de mama [@Hope2004]. Su capacidad
...
técnicas presentan limitaciones diagnósticas [@Pennati2023].
...
realizarse de manera repetida sin exposición acumulativa [@Mann2022].
...
consolidadas [@Rezanejad2022]. La falta de estandarización en las
...
desempeño uniforme entre distintos sistemas [@Ganesan2023], en
...
poblaciones de mayor tamaño [@Rezanejad2022]. A pesar de los avances
...
cáncer de mama [@Rezanejad2022; @Ganesan2023]. En el ámbito técnico, las
...
[@Ganesan2023; @Mansouri2022], hasta matrices planares de
trans-admitancia de 60 x 60 electrodos [@Zhao2014] y algoritmos de
reconstrucción basados en Gauss-Newton, Variación Total y clasificadores
de Machine Learning [@Ganesan2023; @Rixen2023]. A partir de este
...
[@Pennati2023], cada propuesta responde a compromisos particulares entre
complejidad circuital, tiempo de cómputo y resolución espacial
[@Ganesan2023; @Mansouri2022].
...
sobresaliente (82,04% a 96%) [@Rezanejad2022]. Aunque su sensibilidad no
...
[@Mann2022; @ACS2026], su alta especificidad fundamenta su capacidad
para diferenciar de forma no invasiva entre carcinomas malignos y
alteraciones benignas como quistes o fibroadenomas [@Rezanejad2022]. No
...
de impedancia patológica [@Rezanejad2022; @Hope2004].
...
independiente [@Rezanejad2022; @Hope2004]. Su carácter no ionizante,
...
[@Mann2022; @Rezanejad2022], además de contribuir a la reducción de
...
problema inverso clásico [@Rezanejad2022; @Pennati2023; @Rixen2023].

::: thebibliography
99

Organización Mundial de la Salud, "Cáncer de mama," OMS, 3 jul. 2026.
\[En línea\]. Disponible:
<https://www.who.int/es/news-room/fact-sheets/detail/breast-cancer>

International Agency for Research on Cancer, "Breast," *Global Cancer
Observatory: Cancer Today*. \[En línea\]. Disponible:
<https://gco.iarc.who.int/today/en/fact-sheets-cancers/20/breast>

P. A. Ruiz de Viñaspre Alvear, J. L. Soto Noriega, H. Paredes Farto, and
N. Aliaga Molina, "Detección precoz del cáncer de mama: impacto clínico,
sanitario y desafíos en Chile," *Revista Médica Clínica Las Condes*,
vol. 37, no. 2, pp. 162--169, 2026. doi:10.1016/j.rmclc.2026.04.005.

Instituto Nacional del Cáncer, "Exámenes de detección del cáncer de mama
(seno)," National Cancer Institute, 2 dic. 2025. \[En línea\].
Disponible: <https://www.cancer.gov/espanol/tipos/seno/deteccion-mama>

R. M. Mann *et al.*, "Breast cancer screening in women with extremely
dense breasts recommendations of the European Society of Breast Imaging
(EUSOBI)," *European Radiology*, vol. 32, no. 6, pp. 4036--4045, 2022.
doi:10.1007/s00330-022-08617-6.

American Cancer Society, "Limitations of Mammograms," American Cancer
Society, 23 jul. 2026. \[En línea\]. Disponible:
...
M. E. Keemers-Gels, R. P. R. Groenendijk, J. H. M. van den Heuvel,
C. Boetes, P. G. M. Peer, and T. H. Wobbes, "Pain experienced by women
attending breast cancer screening," *Breast Cancer Research and
Treatment*, vol. 60, no. 3, pp. 235--240, 2000.
doi:10.1023/A:1006457520996.

K. K. Engelman, A. M. Cizik, and E. F. Ellerbeck, "Women's satisfaction
with their mammography experience: Results of a qualitative study,"
*Women & Health*, vol. 42, no. 4, pp. 17--35, 2005.
doi:10.1300/J013v42n04_02.

C. C. Mendat, D. Mislan, and L. Hession-Kunz, "Patient comfort from the
technologist perspective: Factors to consider in mammographic imaging,"
*International Journal of Women's Health*, vol. 9, pp. 359--364, 2017.
doi:10.2147/IJWH.S129817.

J. Uscher, "Sugerencias para reducir el dolor durante las mamografías
(mastografías) y después de ellas," Breastcancer.org, 29 oct. 2025.
\[En línea\]. Disponible:
<https://www.breastcancer.org/es/pruebas-deteccion/mamografias/dolor>

J. Pairo-Cuba, N. Saravia-Bastías, M. Soto-Tapia, and
S. Riquelme-Quintana, "Cáncer de mama en Chile: tendencias de mortalidad
en mujeres 2010--2024 y proyecciones hasta 2050," *J. Health Med. Sci.*,
vol. 11, no. extraordinario, pp. 45--53, 2025.

A. Dois, P. Bravo, L. Fernández-González, and C. Uribe, "Consideraciones
para comunicar riesgos y beneficios de la mamografía a mujeres desde la
perspectiva de los expertos," *Revista Médica de Chile*, vol. 149,
no. 2, pp. 196--202, 2021. doi:10.4067/S0034-98872021000200196.

Z. Rezanejad Gatabi, M. Mirhoseini, N. Khajeali, I. Rezanezhad Gatabi,
M. Dabbaghianamiri, and S. Dorri, "The Accuracy of Electrical Impedance
Tomography for Breast Cancer Detection: A Systematic Review and
Meta-Analysis," *The Breast Journal*, vol. 2022, art. no. 8565490, 2022.
doi:10.1155/2022/8565490.

A. Ganesan and V. Durgamahanthi, "Non-Invasive Breast Cancer Detection
Using Electrical Impedance Tomography: Design, Analysis and Comparison
of Reconstruction Algorithms," *Traitement du Signal*, vol. 40, no. 6,
pp. 2809--2817, 2023. doi:10.18280/ts.400641.

F. Pennati *et al.*, "Electrical Impedance Tomography: From the
Traditional Design to the Novel Frontier of Wearables," *Sensors*,
vol. 23, no. 3, art. no. 1182, 2023. doi:10.3390/s23031182.

T. A. Hope and S. E. Iles, "Technology review: The use of electrical
impedance scanning in the detection of breast cancer," *Breast Cancer
Research*, vol. 6, no. 2, pp. 69--74, 2004. doi:10.1186/bcr744.

M. Zhao, H. Wi, E. J. Lee, E. J. Woo, and T. I. Oh, "Feasibility of
anomaly detection and characterization using trans-admittance
mammography with $60 \times 60$ electrode array," *Physics in Medicine &
Biology*, vol. 59, no. 19, pp. 5831--5847, 2014.
doi:10.1088/0031-9155/59/19/5831.

B. Scholz and R. Anderson, "On electrical impedance
scanning---Principles and simulations," *Electromedica*, vol. 68, no. 1,
pp. 35--44, 2000. \[En línea\]. Disponible:
<https://www.biophysicssite.com/Documents/Siemens_EIT.pdf>

H. Mansouri, A. Brik, A. Zitouni, and M. C. E. Yagoub, "3D simulation of
electrical impedance tomography for breast cancer detection using
different electrode configurations," *Medical Technologies Journal*,
vol. 6, no. 1, pp. 885--893, 2022.

J. Rixen, N. Blass, S. Lyra, and S. Leonhardt, "Comparison of Machine
Learning Classifiers for the Detection of Breast Cancer in an Electrical
Impedance Tomography Setup," *Algorithms*, vol. 16, no. 11,
art. no. 517, 2023. doi:10.3390/a16110517.

F. Xu, M. Li, J. Li, and H. Jiang, "Diagnostic accuracy and prognostic
value of three-dimensional electrical impedance tomography imaging in
patients with breast cancer," *Gland Surgery*, vol. 10, no. 9,
pp. 2673--2685, 2021. doi:10.21037/gs-21-348.

J. C. Gómez-Cortés, J. J. Díaz-Carmona, J. A. Padilla-Medina,
A. Espinosa Calderon, A. I. Barranco Gutiérrez, M. Gutiérrez-López, and
J. Prado-Olivarez, "Electrical Impedance Tomography Technical
Contributions for Detection and 3D Geometric Localization of Breast
Tumors: A Systematic Review," *Micromachines*, vol. 13, no. 4,
art. no. 496, 2022. doi:10.3390/mi13040496.

Y. Cheng and M. Fu, "Dielectric properties for non-invasive detection of
normal, benign, and malignant breast tissues using microwave theories,"
*Thorac. Cancer*, vol. 9, no. 4, pp. 459--465, 2018.
